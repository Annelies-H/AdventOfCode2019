...
runIntCode input program = compute program 0 input [] 0
    where
      compute program pointer input output base
          | opCode == 99 = reverse output
            --end program
          | opCode == 1 = compute (runOpCode1 (checkMemory program pointer) pointer instruction base) (pointer + 4) input output base
            -- addition
          | opCode == 2 = compute (runOpCode2 (checkMemory program pointer) pointer instruction base) (pointer + 4) input output base
            -- multiplication
          | opCode == 3 = compute (runOpCode3 (checkMemory program pointer) pointer instruction (head input) base) (pointer + 2) (tail input) output base
            -- store input
          | opCode == 4 = compute program (pointer + 2) input ((runOpCode4 (checkMemory program pointer) pointer instruction base):output) base
            -- output value
          | opCode == 5 = compute program (runOpCode5 (checkMemory program pointer) pointer instruction base) input output base
            -- jump if not zero
          | opCode == 6 = compute program (runOpCode6 (checkMemory program pointer) pointer instruction base) input output base
            -- jump if zero
          | opCode == 7 = compute (runOpCode7 (checkMemory program pointer) pointer instruction base) (pointer + 4) input output base
            -- check less than
          | opCode == 8 = compute (runOpCode8 (checkMemory program pointer) pointer instruction base) (pointer + 4) input output base
            -- cehck equal to
          | opCode == 9 = compute program (pointer + 2) input output (runOpCode9 (checkMemory program pointer) pointer instruction base)
...
runOpCode3 program pointer instruction input base = (checkMemory program param1) V.// [(param1, input)]
    where
